...
from .inputs import _select_anndata_matrix
...
def _worker_function(
    ...
    **kwargs
    ...
    if is_backed:
        ...
        try:
            matrix, _ = _select_anndata_matrix(
                adata_backed, ExpressionSource.parse(expression_source)
            )
            expression_vector = matrix[:, feature_index]
...
def _run_profiling_engine(
    expression_data: Union[np.ndarray, spmatrix, AnnData, ResolvedExpression],
    ...
    if isinstance(expression_data, ResolvedExpression):
        is_backed = expression_data.is_backed
        expression_source = expression_data.source_descriptor
        expr_data_for_worker = (
            str(expression_data.backing_path)
            if is_backed
            else expression_data.expression_matrix
        )
    else:
    ...
    per_condition_df['expression_source'] = expression_source
